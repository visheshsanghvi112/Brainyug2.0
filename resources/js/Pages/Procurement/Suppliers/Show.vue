...
const props = defineProps({
    ...
    invoiceSettlementOptions: Array,
    paymentAllocationMap: Object,
    recentReturns: Array,
    ledgerUrl: String,
});
...
const reallocationForm = useForm({
    reason: '',
    reallocation_date: new Date().toISOString().slice(0, 10),
    allocations: [],
});
...
const outstandingLabel = computed(() =>
    Number(props.summary?.current_balance || 0) > 0 ? 'Outstanding Payable' : 'No Outstanding Payable'
);

const openInvoiceLabel = computed(() =>
    Number(props.summary?.open_invoice_count || 0) > 0 ? 'Open Invoice Exposure' : 'All Approved Invoices Settled'
);
...
function currentAllocationsForPayment(entry) {
    return props.paymentAllocationMap?.[entry.id] || [];
}

function allocationCapacity(invoiceId) {
    const invoice = (props.invoiceSettlementOptions || []).find((item) => Number(item.id) === Number(invoiceId));
    const currentAllocation = Number(
        reallocationForm.allocations.find((item) => Number(item.purchase_invoice_id) === Number(invoiceId))?.amount || 0
    );

    return Number(invoice?.outstanding_amount || 0) + currentAllocation;
}

function openReallocationModal(entry) {
    selectedPaymentEntry.value = entry;
    reallocationForm.clearErrors();
    reallocationForm.reason = '';
    reallocationForm.reallocation_date = new Date().toISOString().slice(0, 10);
    reallocationForm.allocations = (props.invoiceSettlementOptions || [])
        .map((invoice) => {
            const currentAmount = Number(
                currentAllocationsForPayment(entry).find((item) => Number(item.purchase_invoice_id) === Number(invoice.id))?.allocated_amount || 0
            );
        ...
        .filter((row) => {
            const invoice = (props.invoiceSettlementOptions || []).find((item) => Number(item.id) === Number(row.purchase_invoice_id));
            return Number(row.amount || 0) > 0 || Number(invoice?.outstanding_amount || 0) > 0;
        });
...
function submitReallocation() {
    const paymentAmount = Number(selectedPaymentEntry.value?.debit || 0);
    const requestedTotal = reallocationForm.allocations.reduce((sum, row) => sum + Number(row.amount || 0), 0);

    reallocationForm.clearErrors();

    if (!reallocationForm.reason.trim()) {
        reallocationForm.setError('reason', 'Reallocation reason is required.');
        return;
    }

    if (Math.abs(requestedTotal - paymentAmount) > 0.009) {
        reallocationForm.setError('allocations', `Allocation total must match the payment amount of ${formatMoney(paymentAmount)}.`);
        return;
    }

    reallocationForm.transform((data) => ({
        ...data,
    ...
    })).post(route('admin.suppliers.payments.reallocate', [props.supplier.id, selectedPaymentEntry.value.id]), {
        preserveScroll: true,
        onSuccess: () => {
...
function reversePayment(entry) {
    const reason = window.prompt('Why are you reversing this supplier payment?', entry.reversal_reason || 'Wrong payment posted');
    ...
    }).post(route('admin.suppliers.payments.reverse', [props.supplier.id, entry.id]), {
        preserveScroll: true,
    });
}
</script>
    ...
    <AuthenticatedLayout>
            ...
            <div class="mx-auto max-w-7xl space-y-6 sm:px-6 lg:px-8">
                <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
                    ...
                    <div class="rounded-2xl border border-indigo-200 bg-white p-5 shadow-sm dark:border-indigo-900/40 dark:bg-gray-800">
                        <p class="text-xs font-semibold uppercase tracking-[0.2em] text-indigo-600 dark:text-indigo-300">{{ openInvoiceLabel }}</p>
                        <p class="mt-3 text-2xl font-bold text-gray-900 dark:text-white">{{ formatMoney(summary.open_invoice_exposure) }}</p>
                        <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">{{ summary.open_invoice_count }} approved invoice(s) still open after returns and payments.</p>
                    </div>
                    <div class="rounded-2xl border border-sky-200 bg-white p-5 shadow-sm dark:border-sky-900/40 dark:bg-gray-800">
                        <p class="text-xs font-semibold uppercase tracking-[0.2em] text-sky-600 dark:text-sky-300">Approved Purchases</p>
                    ...
                    <div class="rounded-2xl border border-rose-200 bg-white p-5 shadow-sm dark:border-rose-900/40 dark:bg-gray-800">
                        ...
                        <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">{{ summary.overdue_invoices }} invoice(s) still overdue after returns and payments.</p>
                    </div>
                </div>
                ...
                <div class="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
                    <div class="space-y-6">
                                ...
                                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                    ...
                                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700/60">
                                        <tr v-if="!recentLedgers.length">
                                            <td colspan="7" class="px-6 py-10 text-center text-sm text-gray-400 dark:text-gray-500">No ledger entries recorded yet.</td>
                                        </tr>
                                        <tr v-for="entry in recentLedgers" :key="entry.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/20">
                                            ...
                                            <td class="px-6 py-4">
                                                ...
                                                <div v-if="entry.reversed_at" class="mt-1 inline-flex rounded-full bg-blue-100 px-2 py-0.5 text-[11px] font-semibold text-blue-700">
                                                    Reversed
                                                </div>
                                            </td>
                                            <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300">
                                                ...
                                                <div v-if="entry.reversal_reason" class="mt-1 text-xs text-blue-600 dark:text-blue-300">Reason: {{ entry.reversal_reason }}</div>
                                            </td>
                                            ...
                                            <td class="px-6 py-4 text-right">
                                                <button
                                                    v-if="entry.transaction_type === 'PAYMENT_MADE' && !entry.reversed_at && Number(entry.supplier_payment_allocations_count || 0) > 0"
                                                    type="button"
                                                    class="inline-flex items-center gap-1 rounded-lg bg-slate-100 px-3 py-1.5 text-xs font-semibold text-slate-700 transition hover:bg-slate-200 dark:bg-slate-700 dark:text-slate-100 dark:hover:bg-slate-600"
                                                    @click="openReallocationModal(entry)"
                                                >
                                                ...
                                                <button
                                                    v-if="entry.transaction_type === 'PAYMENT_MADE' && !entry.reversed_at && Number(entry.supplier_payment_allocations_count || 0) > 0"
                                                    type="button"
                                                    class="ml-2 inline-flex items-center gap-1 rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-blue-500"
                                                    @click="reversePayment(entry)"
                                                >
                        ...
                        <section class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-gray-200 dark:bg-gray-800 dark:ring-gray-700">
                            <div class="flex items-center justify-between border-b border-gray-100 px-6 py-4 dark:border-gray-700">
                                <div>
                                    <h3 class="text-base font-semibold text-gray-900 dark:text-white">Invoice Settlement Desk</h3>
                                    <p class="text-sm text-gray-500 dark:text-gray-400">Approved supplier bills with return adjustments, payments, and live outstanding.</p>
                                </div>
                                ...
                                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                    <thead class="bg-gray-50 dark:bg-gray-900/50">
                                        <tr>
                                            ...
                                            <th class="px-6 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Gross</th>
                                            <th class="px-6 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Returns</th>
                                    ...
                                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700/60">
                                        <tr v-if="!recentInvoices.length">
                                            <td colspan="7" class="px-6 py-10 text-center text-sm text-gray-400 dark:text-gray-500">No approved purchase invoices yet.</td>
                                        </tr>
                                        <tr v-for="invoice in recentInvoices" :key="invoice.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/20">
                                            <td class="px-6 py-4 text-sm text-gray-700 dark:text-gray-200">
                                                ...
                                                <div v-if="invoice.is_overdue" class="mt-1 inline-flex rounded-full bg-rose-100 px-2 py-0.5 text-[11px] font-semibold text-rose-700">
                                                    Overdue
                                                </div>
                                                <div v-else-if="Number(invoice.outstanding_amount || 0) <= 0" class="mt-1 inline-flex rounded-full bg-emerald-100 px-2 py-0.5 text-[11px] font-semibold text-emerald-700">
                                                    Settled
                                                </div>
                                            </td>
                                            ...
                                            <td class="px-6 py-4 text-right text-sm font-semibold text-gray-900 dark:text-white">{{ formatMoney(invoice.gross_amount) }}</td>
                                            <td class="px-6 py-4 text-right text-sm font-semibold text-amber-600">{{ Number(invoice.return_adjusted_amount || 0) ? formatMoney(invoice.return_adjusted_amount) : '-' }}</td>
                                            <td class="px-6 py-4 text-right text-sm font-semibold text-emerald-600">{{ Number(invoice.paid_amount || 0) ? formatMoney(invoice.paid_amount) : '-' }}</td>
                                            <td class="px-6 py-4 text-right text-sm font-bold" :class="Number(invoice.outstanding_amount || 0) > 0 ? 'text-rose-600' : 'text-gray-900 dark:text-white'">{{ formatMoney(invoice.outstanding_amount) }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
        ...
        <div v-if="showPaymentModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl dark:bg-gray-800">
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Post a payment and auto-settle the oldest open approved invoices first.</p>
                ...
                <form class="mt-6 space-y-4" @submit.prevent="submitPayment">
                    <div>
                        ...
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Current ledger balance: {{ formatMoney(summary.current_balance) }} | Open invoice exposure: {{ formatMoney(summary.open_invoice_exposure) }}</p>
        ...
        <div v-if="showReallocateModal && selectedPaymentEntry" class="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div class="absolute inset-0 bg-black/50" @click="showReallocateModal = false" />
            <div class="relative max-h-[90vh] w-full max-w-4xl overflow-hidden rounded-2xl bg-white shadow-2xl dark:bg-gray-800">
                <div class="border-b border-gray-100 px-6 py-4 dark:border-gray-700">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Reallocate Supplier Payment</h3>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        Correct how {{ formatMoney(selectedPaymentEntry.debit) }} is distributed across approved invoices without reversing the full payment.
                    </p>
                </div>

                <form class="space-y-4 p-6" @submit.prevent="submitReallocation">
                    <div class="grid gap-4 md:grid-cols-2">
                        <div>
                            <label class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Reallocation Date</label>
                            <input v-model="reallocationForm.reallocation_date" type="date" class="w-full rounded-xl border-0 bg-gray-50 px-4 py-3 text-sm ring-1 ring-inset ring-gray-200 focus:ring-2 focus:ring-slate-500 dark:bg-gray-700 dark:text-white dark:ring-gray-600" />
                            <p v-if="reallocationForm.errors.reallocation_date" class="mt-1 text-xs text-red-500">{{ reallocationForm.errors.reallocation_date }}</p>
                        </div>
                        <div>
                            <label class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Reason</label>
                            <input v-model="reallocationForm.reason" type="text" class="w-full rounded-xl border-0 bg-gray-50 px-4 py-3 text-sm ring-1 ring-inset ring-gray-200 focus:ring-2 focus:ring-slate-500 dark:bg-gray-700 dark:text-white dark:ring-gray-600" placeholder="Example: payment tagged to wrong invoice" />
                            <p v-if="reallocationForm.errors.reason" class="mt-1 text-xs text-red-500">{{ reallocationForm.errors.reason }}</p>
                        </div>
                    </div>

                    <div class="rounded-2xl border border-gray-200 dark:border-gray-700">
                        <div class="grid grid-cols-[2fr,1fr,1fr,1fr] gap-3 border-b border-gray-100 bg-gray-50 px-4 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:border-gray-700 dark:bg-gray-900/40 dark:text-gray-400">
                            <div>Invoice</div>
                            <div class="text-right">Open Capacity</div>
                            <div class="text-right">Current Allocation</div>
                            <div class="text-right">New Allocation</div>
                        </div>
                        <div class="max-h-[380px] overflow-y-auto">
                            <div
                                v-for="row in reallocationForm.allocations"
                                :key="row.purchase_invoice_id"
                                class="grid grid-cols-[2fr,1fr,1fr,1fr] gap-3 border-b border-gray-100 px-4 py-3 text-sm last:border-b-0 dark:border-gray-700"
                            >
                                <div>
                                    <div class="font-semibold text-gray-900 dark:text-white">
                                        {{ (invoiceSettlementOptions.find((item) => Number(item.id) === Number(row.purchase_invoice_id)) || {}).invoice_number }}
                                    </div>
                                    <div class="text-xs text-gray-500 dark:text-gray-400">
                                        {{ (invoiceSettlementOptions.find((item) => Number(item.id) === Number(row.purchase_invoice_id)) || {}).supplier_invoice_no || 'No supplier invoice no.' }}
                                    </div>
                                </div>
                                <div class="text-right font-semibold text-rose-600">
                                    {{ formatMoney(allocationCapacity(row.purchase_invoice_id)) }}
                                </div>
                                <div class="text-right font-semibold text-sky-600">
                                    {{
                                        formatMoney(
                                            currentAllocationsForPayment(selectedPaymentEntry).find((item) => Number(item.purchase_invoice_id) === Number(row.purchase_invoice_id))?.allocated_amount || 0
                                        )
                                    }}
                                </div>
                                <div>
                                    <input
                                        ...
                                        min="0"
                                        step="0.01"
                                        :max="allocationCapacity(row.purchase_invoice_id)"
                                        class="w-full rounded-xl border-0 bg-gray-50 px-3 py-2 text-right text-sm ring-1 ring-inset ring-gray-200 focus:ring-2 focus:ring-slate-500 dark:bg-gray-700 dark:text-white dark:ring-gray-600"
                                    />
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="flex items-center justify-between rounded-xl bg-slate-50 px-4 py-3 text-sm dark:bg-slate-900/40">
                        <div class="text-gray-600 dark:text-gray-300">
                            Payment Amount: <span class="font-semibold text-gray-900 dark:text-white">{{ formatMoney(selectedPaymentEntry.debit) }}</span>
                        </div>
                        <div class="text-gray-600 dark:text-gray-300">
                            Allocated Total:
                            <span class="font-semibold text-gray-900 dark:text-white">
                                {{ formatMoney(reallocationForm.allocations.reduce((sum, row) => sum + Number(row.amount || 0), 0)) }}
                            </span>
                        </div>
                    </div>
                    <p v-if="reallocationForm.errors.allocations" class="text-xs text-red-500">{{ reallocationForm.errors.allocations }}</p>
                    <p v-if="reallocationForm.errors.payment_reallocation" class="text-xs text-red-500">{{ reallocationForm.errors.payment_reallocation }}</p>

                    <div class="flex justify-end gap-3">
                        <button type="button" class="rounded-lg px-4 py-2 text-sm font-semibold text-gray-700 ring-1 ring-gray-300 hover:bg-gray-50 dark:text-gray-300 dark:ring-gray-600 dark:hover:bg-gray-700" @click="showReallocateModal = false">
                            Cancel
                        </button>
                        <button type="submit" :disabled="reallocationForm.processing" class="rounded-lg bg-slate-900 px-5 py-2 text-sm font-semibold text-white hover:bg-slate-800 disabled:opacity-50 dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-white">
                            Save Allocation
                        </button>
                    </div>
                </form>
