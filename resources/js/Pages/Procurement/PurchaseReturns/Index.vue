                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-700/50">
                            ...
                            >
                                <td class="whitespace-nowrap px-6 py-4">
                                    <div class="font-bold text-gray-900 dark:text-white">{{ ret.return_number }}</div>
                                    <div class="text-xs text-gray-500 dark:text-gray-400">{{ ret.return_date }}</div>
                                </td>
                                <td class="whitespace-nowrap px-6 py-4">
                                    <div class="font-medium text-gray-900 dark:text-white">{{ ret.supplier?.name || '—' }}</div>
                                    <div class="text-xs text-blue-500 hover:underline">
                                        <Link v-if="ret.purchase_invoice_id" :href="route('admin.purchase-invoices.show', ret.purchase_invoice_id)">
                                            Inv: {{ ret.purchase_invoice?.invoice_number }}
                                        </Link>
                                    </div>
                                </td>
                                <td class="whitespace-nowrap px-6 py-4">
                                    <div class="text-sm font-semibold text-gray-900 dark:text-white">₹{{ ret.total_amount }}</div>
                                </td>
                                <td class="whitespace-nowrap px-6 py-4 text-center">
                                    <span :class="statusColors[ret.status]" class="inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium capitalize">
                                        {{ ret.status }}
                                    </span>
                                </td>
                                <td class="whitespace-nowrap px-6 py-4 text-center">
                                    <Link
                                        :href="route('admin.purchase-returns.show', ret.id)"
