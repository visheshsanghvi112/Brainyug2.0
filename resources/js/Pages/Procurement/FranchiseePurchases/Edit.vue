...
const form = useForm({
    ...
    received_date: props.purchase.received_date || '',
    reason_code: props.purchase.reason_code || 'normal',
...
const submit = () => {
    ...
    form.put(route('admin.franchisee-purchases.update', props.purchase.id));
};
</script>

<template>
    <Head title="Edit Outside Purchase" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Outside Purchase {{ purchase.transaction_number }}</h2>
        </template>
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-5">
                <div class="bg-white rounded-xl border border-gray-100 p-4">
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div>
                            ...
                            <input :value="purchase.transaction_number" disabled class="w-full rounded-md border-gray-300 bg-gray-50" />
                        </div>
                        <div>
                            <label class="block text-sm font-medium mb-1">Franchisee</label>
                            <input :value="purchase.franchisee?.shop_name || '-'" disabled class="w-full rounded-md border-gray-300 bg-gray-50" />
                        </div>
                        <div>
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex gap-2">
                        <Link :href="route('admin.franchisee-purchases.show', purchase.id)" class="rounded-lg bg-gray-100 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-200">Cancel</Link>
                        <button type="button" @click="submit" :disabled="form.processing" class="rounded-lg bg-emerald-600 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-500 disabled:opacity-60">Update Draft</button>
                    </div>
                </div>
            </div>
        </div>
